...
function argumentFailureFingerprint(block) {
  if (!block?.isError) return undefined
  const text = toolResultText(block).replace(/\s+/g, ' ').trim().toLowerCase()
  if (!/(?:invalid arguments?|missing required propert|must be (?:an?|one of)|expected (?:an?|type))/.test(text)) return undefined
  return text
    .replace(/\[\d+\]/g, '[]')
...
function repeatedArgumentFailureState(messages, turnStart, threshold = 2) {
  const callNames = new Map()
  ...
  for (const message of (messages ?? []).slice(turnStart + 1)) {
    for (const block of message.content ?? []) {
      ...
      const fingerprint = argumentFailureFingerprint(block)
  ...
  return { failedTools, maxFailures }
}
...
export function prepareSearchConvergence(options = {}, maxCalls = 4) {
  ...
  const argumentFailures = repeatedArgumentFailureState(options.messages, turnStart)
  if (!hasLegacyWebTools && !hasGeminiWebSearch && !duplicateTool && !emptyDiscovery.shouldConverge && !argumentFailures.failedTools.size) return options
  let guidance = hasLegacyWebTools || hasGeminiWebSearch ? SEARCH_CONVERGENCE_POLICY : ''
  ...
  if (argumentFailures.failedTools.size) {
    guidance += `\n- The following tools repeatedly failed argument validation and are temporarily unavailable for this turn: ${[...argumentFailures.failedTools].join(', ')}. Do not rephrase and retry the same invalid call. Continue with a different relevant tool, or explain the blocker if no safe alternative exists.`
  }
  return {
    ...options,
    ...
    tools: tools
      ...
      .filter((tool) => !argumentFailures.failedTools.has(tool.name))
