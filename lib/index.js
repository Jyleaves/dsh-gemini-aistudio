...
function latestPriorSearchSources(messages) {
  for (let index = (messages?.length ?? 0) - 1; index >= 0; index -= 1) {
    const sources = []
    for (const block of messages[index]?.content ?? []) {
      if (block.type !== 'tool-result' || block.isError) continue
      const text = toolResultText(block)
      for (const match of text.matchAll(/^\d+\.\s+(https?:\/\/\S+?)(?:\s+—\s+(.+?))?\s+\[(?:google_search|url_context)\]\s*$/gmu)) {
        sources.push({ uri: match[1], title: (match[2] ?? '').trim() })
      }
    }
...
function isHumanUserMessage(message) {
  ...
  return (message.content ?? []).some((block) => ['text', 'reasoning', 'image'].includes(block.type))
}
...
class GeminiAdapter {
  ...
  async *stream(options) {
    ...
    const toolContext = {
      ...
      latestUserText,
      priorSearchSources: latestPriorSearchSources(requestOptions.messages),
      now: new Date(),
    }
