...
function recoverableFailureFingerprint(block) {
  ...
  if (!/(?:invalid arguments?|missing required propert|must be (?:an?|one of)|expected (?:an?|type)|sandbox escalation .* not strictly wider|old_string was not found|include must be one glob|more raw output than .* cap|not lossless json)/.test(text)) return undefined
  return text
    .replace(/\[\d+\]/g, '[]')
...
export async function runNativeWebLookup(config, query, signal) {
  ...
  return { text: text.slice(0, maxOutputChars), model, query: normalizedQuery, googleSearch: true, urlContext: true }
}
...
class GeminiAdapter {
  ...
  async *stream(options) {
    ...
    const toolContext = { currentSandboxMode: runtimeSandboxMode(requestOptions) }
